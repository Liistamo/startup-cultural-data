...
"""
05_lcp_csv_to_geojson

Läser alla CSV-filer i ./lcp_data/ (rekursivt) och exporterar en GeoJSON
med EN punktlista, där ENDAST rader med praesidia == 1 inkluderas.

Utdata:
  - Huvudfil: lcp_final/lcp_final_places_YYYY-MM-DD_HH-MM-SS.geojson
  - Skipplogg: lcp_final/skipped_rows_YYYY-MM-DD_HH-MM-SS.csv

Övrigt:
  - Boundary-filter + whitelist stöds (geojson/boundaries/*.geojson).
  - Kategori-normalisering, färg, URL-normalisering m.m. som tidigare.
"""
...
# --- Whitelist för namn utanför boundary ---
...
ALIASES = {
    ...
    # behåller originalnamn "praesidia"
}
...
# --- CSV-läsare (komma-separerad) ---
def read_csv_file(path: Path) -> list[dict]:
    text = path.read_text(encoding="utf-8-sig", errors="replace")
    rows = list(csv.reader(text.splitlines(), delimiter=","))  # CSV, ej TSV
    if not rows:
        return []
...
def normalize_category(raw_cat: str | None) -> str:
    ...
    # Fallback: säkerställ exakt EN "Startup" som prefix.
    # Ta bort valfritt antal ledande "Startup" (case-insensitive) + ev. avskiljare.
    tail = re.sub(r'(?i)^(?:startup[\s_-]*)+', '', original).strip()
    return "Startup" if not tail else f"Startup {tail}"
...
def main():
    ...
    print(f"📄 Hittade {len(csv_paths)} CSV-fil(er).")
    if not csv_paths:
        print(f"❌ Inga CSV-filer i {INPUT_DIR}/", file=sys.stderr); sys.exit(1)
    ...
    out_geojson = OUTPUT_DIR / f"lcp_final_places_{ts}.geojson"
    ...
    print(f"   Whitelist (outside boundary): {len(OUTSIDE_NAME_WHITELIST)} namn")
